...
<body>
	...
	// Units required
	String units_required_str ="SELECT upper_division_units, lower_division_units, core_units, technical_units, major_elective_units, college_units from undergraduateDegree where degree_id=?;"; 
	/*
	...
	String core_units_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment where underGradudateDegreeRequiarment.degree_id=? and underGradudateDegreeRequiarment.is_core='YES' and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and total_enrollment.student_id =?; ";

	String technical_elective_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment where underGradudateDegreeRequiarment.degree_id=? and underGradudateDegreeRequiarment.is_technical_elective='YES' and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and total_enrollment.student_id = ?;"; 

	String major_elective_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment where underGradudateDegreeRequiarment.degree_id=? and underGradudateDegreeRequiarment.is_major_elective='YES' and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and total_enrollment.student_id = ?;"; 

	String college_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment where underGradudateDegreeRequiarment.degree_id=? and underGradudateDegreeRequiarment.is_college_required='YES' and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and total_enrollment.student_id = ?;"; 

	String upperUnits_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment, courses where underGradudateDegreeRequiarment.degree_id=? and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and courses.course_number = total_enrollment.course_number and courses.is_upper = 1 and total_enrollment.student_id = ?;"; 

	String lowerUnits_taken_queryStr = "WITH total_enrollment as ( (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM enrollment) UNION (SELECT student_id, course_number, year_, quarter, section_id, faculty_name, unit_taken FROM past_enrollment) ) SELECT sum(total_enrollment.unit_taken) FROM underGradudateDegreeRequiarment, total_enrollment, courses where underGradudateDegreeRequiarment.degree_id=? and total_enrollment.course_number = underGradudateDegreeRequiarment.course_number and courses.course_number = total_enrollment.course_number and courses.is_upper = 0 and total_enrollment.student_id = ?;"; 
	...
	if (action != null && action.equals("query")) {
		...
		*/
		...
		units_core = units_required_RS.getInt("core_units");
		units_technical_elective = units_required_RS.getInt("technical_units");
		units_major_elective = units_required_RS.getInt("major_elective_units");
		units_college_required = units_required_RS.getInt("college_units");
